    ...
    def process(self, message:DocumentWrapper) -> DocumentWrapper:
        try:
            ...
            completion = client.beta.chat.completions.parse(
                model=MODEL_NAME,
                messages=[
                    {"role": "system", "content": "Extract the patient information."},
                    {"role": "user", "content": message.content},
                ],
                response_format=PatientObject,
            )
            
            patient = completion.choices[0].message.parsed
            if patient: 
                logger.info(f"analysis type: {type(patient)}")
                logger.info("------------------------")                
                logger.info(f"Last Name: {patient.last_name}")
                logger.info(f"First Name:   {patient.first_name}")
                logger.info(f"Date of Birth: {patient.date_of_birth}")       
